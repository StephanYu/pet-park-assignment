...
contract PetParkTest is Test, PetPark {
    ...
    function testCannotAddAnimalWhenNonOwner() public {
        address nonOwner = testPrimaryAccount;
        petPark.add{from: nonOwner}(AnimalType.Cat, 5);
        assertEq(petPark.animalCounts(AnimalType.Cat), 0);
    }
    ...
    function testBorrowCountDecrement() public {
        ...
        petPark.borrow(24, Gender.Male, AnimalType.Fish);
        uint reducedCount = petPark.animalCounts(AnimalType.Fish);

        assertEq(reducedCount, initialCount - 1);
    }
